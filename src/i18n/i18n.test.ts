...
describe("i18n Engine", () => {
  ...
  describe("getTranslation", () => {
    ...
    it("retrieves deeply nested keys correctly", () => {
      ...
      expect(getTranslation("contact.submitButton")).toBe(
        "Ignite Conversation",
      );
    });

    it("retrieves the new blog index titles and descriptions", () => {
      setLanguage("en");
      expect(getTranslation("blog.indexTitlePrefix")).toBe("The ");
      expect(getTranslation("blog.indexTitleHighlight")).toBe("Ember");
      expect(getTranslation("blog.indexTitleSuffix")).toBe(" Blog");
      expect(getTranslation("blog.indexDescription")).toBe(
        "Deep dives into game development, architectural patterns, and the future of digital experiences.",
      );

      setLanguage("es");
      expect(getTranslation("blog.indexTitlePrefix")).toBe("El Blog de ");
      expect(getTranslation("blog.indexTitleHighlight")).toBe("Ember");
      expect(getTranslation("blog.indexTitleSuffix")).toBe("");
      expect(getTranslation("blog.indexDescription")).toBe(
        "Inmersiones profundas en el desarrollo de juegos, patrones arquitectónicos y el futuro de las experiencias digitales.",
      );
    });
  });
});
